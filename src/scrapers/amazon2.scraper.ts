...
export async function scrapeAmazons(query: string) {
    ...
    try {
        ...
        axios.get('https://app.scrapingbee.com/api/v1/amazon/product', {
            params: {
                'api_key': '16X9CWFULA05N41ADY50H46K1AH5T4ZFIVA4VWPA5YGK2DYOXMA17QKSVUWTES6IAFWNCA8T6K9UAOK7',
                'url': 'YOUR-URL',
                'query': query,
            }
        }).then(function (response) {
            // handle success
            console.log('Scraping Bee: ',response?.data);
        }).catch((error: any) => {
